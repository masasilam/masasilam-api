...
public class ZineController {
    ...
    public ResponseEntity<DataResponse<ZineResponse>> createZine(
            @Valid @ModelAttribute ZineRequest request) {
        DataResponse<ZineResponse> response = zineService.createZine(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @GetMapping("/{slug}")
    public ResponseEntity<DataResponse<ZineResponse>> getZineDetail(
    ...
    @GetMapping
    public ResponseEntity<DatatableResponse<ZineResponse>> getZinesPaginated(
        ...
        DatatableResponse<ZineResponse> response =
                zineService.getPaginatedZines(page, limit, sortField, sortOrder, criteria);
        return ResponseEntity.ok(response);
    }

    @PutMapping(value = "/{id}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<DataResponse<Zine>> updateZine(
    ...
    public ResponseEntity<?> downloadZine(
            @PathVariable String slug, HttpServletRequest request) {
    ...
    public ResponseEntity<DataResponse<List<GenreResponse>>> getAllGenres(
            @RequestParam(defaultValue = "false") boolean includeZineCount) {
