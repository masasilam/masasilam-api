...
import com.naskah.demo.model.dto.request.social.UpdateSocialProfileRequest;
import com.naskah.demo.model.dto.response.*;
import com.naskah.demo.model.dto.response.social.*;
import com.naskah.demo.service.social.SocialProfileService;
...
public class SocialProfileController {
    ...
    public ResponseEntity<DataResponse<UserPublicProfileResponse>> getProfileById(
            @PathVariable Long userId) {
        return ResponseEntity.ok(profileService.getPublicProfile(userId));
    }

    @GetMapping("/{username}")
    public ResponseEntity<DataResponse<UserPublicProfileResponse>> getProfileByUsername(
            @PathVariable String username) {
        return ResponseEntity.ok(profileService.getPublicProfileByUsername(username));
    }

    @PutMapping("/me")
    public ResponseEntity<DataResponse<UserPublicProfileResponse>> updateMyProfile(
    ...
    public ResponseEntity<DataResponse<FollowStatsResponse>> getFollowers(
            @PathVariable Long userId,
    ...
    public ResponseEntity<DataResponse<FollowStatsResponse>> getFollowing(
            @PathVariable Long userId,
            @RequestParam(defaultValue = "1") @Min(1) int page,
            @RequestParam(defaultValue = "20") @Min(1) int limit) {
