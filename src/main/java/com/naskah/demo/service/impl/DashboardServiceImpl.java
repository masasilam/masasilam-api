...
import com.naskah.demo.model.dto.response.ZineDashboardDTOs.ZineInProgressResponse;
...
public class DashboardServiceImpl implements DashboardService {
    ...
    /** Versi zine dari resolveChapter */
    ...
    // ═════════════════════════════════════════════════════════
    // MAIN DASHBOARD — sekarang termasuk zinesInProgress
    // ═════════════════════════════════════════════════════════
    ...
    public DataResponse<DashboardMainResponse> getMainDashboard() {
        ...
        response.setZinesInProgress(buildZinesInProgress(user.getId()));    // ← BARU
    ...
    // ─────────────────────────────────────────────────────────
    // OVERVIEW STATS — waktu baca buku + zine digabung
    // ─────────────────────────────────────────────────────────
    private OverviewStats buildOverviewStats(Long userId) {
        ...
        stats.setTotalBooks((int) (totalBooks + totalZines));   // gabungan
    ...
    // ─────────────────────────────────────────────────────────
    // BOOKS IN PROGRESS (tidak berubah)
    // ─────────────────────────────────────────────────────────
        ...
        return latestSessions.stream()
                .map(session -> {
                    ...
                    ZineReadingProgress progress =
                            zineProgressMapper.findByUserAndZine(userId, zine.getId());
                    double pct = progress != null && progress.getPercentageCompleted() != null
                            ? progress.getPercentageCompleted().doubleValue() : 0.0;
                    if (pct >= 95.0) return null;
    ...
    // ─────────────────────────────────────────────────────────
    // READING PATTERN (buku saja — tidak perlu zine)
    // ─────────────────────────────────────────────────────────
    ...
    private List<RecentlyReadResponse> buildRecentlyRead(Long userId) {
        ...
        // Tambahkan dari zine — reuse RecentlyReadResponse dengan prefix judul
        ...
        for (ZineReadingSession session : latestZinePerZine.values()) {
            ...
            RecentlyReadResponse r = new RecentlyReadResponse();
            r.setBookId(zine.getId());                        // zine id
            r.setBookSlug("zines/" + zine.getSlug());         // prefix agar frontend tahu ini zine
        ...
        // Urutkan by lastReadAt DESC dan limit 6
    ...
    public DataResponse<AnnotationsPageResponse> getAnnotations(
        ...
        if ("all".equals(type) || "highlight".equals(type) || "note".equals(type)) {
            ...
            for (EpubAnnotation ann : epubAnnotations) {
                ...
                // Cari dulu di buku, lalu di zine
                ...
                if (book != null) {
                    ...
                    // Coba cari di zine (book_id di epub_annotation bisa juga zine_id)
        ...
        if ("all".equals(type) || "bookmark".equals(type)) {
            ...
            for (EpubBookmark bm : epubBookmarks) {
                ...
                if (book != null) {
                    item.setBookId(book.getId()); item.setBookSlug(book.getSlug());
                    item.setBookTitle(book.getTitle()); item.setBookCover(book.getCoverImageUrl());
                } else {
    ...
    public DataResponse<LibraryPageResponse> getLibrary(
        ...
        List<LibraryBookResponse> books = latestPerBook.entrySet().stream()
                .map(entry -> {
                    ...
                    r.setBookId(book.getId()); r.setBookSlug(book.getSlug()); r.setBookTitle(book.getTitle());
                    r.setAuthorName(getBookAuthorName(book.getId())); r.setCoverImageUrl(book.getCoverImageUrl());
                    r.setProgressPercentage(pct); r.setReadingStatus(status); r.setLastReadAt(session.getStartedAt());
                    ...
                    r.setNoteCount(0); r.setCurrentChapter(displayChapter);
                    r.setTotalChapters(progress != null && progress.getTotalPages() != null ? progress.getTotalPages() : 0);
                    r.setLastCfi(progress != null ? progress.getCurrentPosition() : null);
                    return r;
                })
        ...
        Comparator<LibraryBookResponse> comparator = switch (sortBy) {
            case "progress" -> Comparator.comparing(LibraryBookResponse::getProgressPercentage, Comparator.reverseOrder());
            case "title"    -> Comparator.comparing(LibraryBookResponse::getBookTitle, Comparator.nullsLast(Comparator.naturalOrder()));
            default         -> Comparator.comparing(LibraryBookResponse::getLastReadAt, Comparator.nullsLast(Comparator.reverseOrder()));
        };
        ...
        pageResponse.setTotalData(total); pageResponse.setPage(page); pageResponse.setLimit(limit);
    ...
    public DataResponse<ReadingHistoryPageResponse> getReadingHistory(int days, int page, int limit) {
        ...
        List<ReadingHistoryItemResponse> items = sessions.stream()
                .map(session -> {
                    ...
                    item.setActivityId(session.getId()); item.setActivityType("reading_session");
                    item.setBookId(book.getId()); item.setBookSlug(book.getSlug()); item.setBookTitle(book.getTitle());
                    item.setAuthorName(getBookAuthorName(book.getId())); item.setBookCover(book.getCoverImageUrl());
        ...
        pageResponse.setTotal(total); pageResponse.setPage(page); pageResponse.setLimit(limit);

        return new DataResponse<>(SUCCESS, "Reading history retrieved", HttpStatus.OK.value(), pageResponse);
    }

    // ═════════════════════════════════════════════════════════
    // STATISTICS (buku — tidak berubah)
    // ═════════════════════════════════════════════════════════
    ...
    public DataResponse<StatisticsResponse> getStatistics(int period) {
        ...
        if (currEstWpm > 0 && prevMinutes > 0) {
            long prevBab = prevSessions.stream().filter(s -> !isEpubSession(s))
                    .mapToInt(s -> { int start = s.getStartChapter() != null ? s.getStartChapter() : 0;
                        int end = s.getEndChapter() != null ? s.getEndChapter() : start;
                        return Math.max(0, Math.abs(end - start) + 1); }).sum()
    ...
    // ═════════════════════════════════════════════════════════
    // CALENDAR — gabungkan sesi buku + zine
    // ═════════════════════════════════════════════════════════
    ...
    public DataResponse<CalendarResponse> getCalendar(int year, int month) {
        ...
        List<ReadingSession> bookSessions =
                sessionMapper.findUserSessionsBetween(user.getId(), start, end);
        List<ZineReadingSession> zineSessions =
                zineSessionMapper.findUserSessionsBetween(user.getId(), start, end);
        ...
        for (ZineReadingSession s : zineSessions) {
            ...
            if (zine != null) {
                // Buat Book proxy supaya CalendarBookEntry bisa diisi
                Book proxy = new Book();
    ...
    // ═════════════════════════════════════════════════════════
    // QUICK STATS — gabungan buku + zine
    // ═════════════════════════════════════════════════════════
    ...
    public DatatableResponse<UserReviewItemResponse> getUserReviews(int page, int limit) {
        ...
        List<UserReviewItemResponse> items = reviews.stream()
                .map(r -> {
                    ...
                    item.setReviewId(r.getId()); item.setBookId(r.getBookId());
                    ...
                    item.setReviewContent(r.getContent()); item.setCreatedAt(r.getCreatedAt());
                    return item;
                }).toList();
    ...
    private int calculateLongestStreak(List<LocalDateTime> allDates) {
        ...
        for (int i = 1; i < dates.size(); i++) {
            ...
            if (days == 1) { maxStreak = Math.max(maxStreak, ++current); } else { current = 1; }
        }
        return maxStreak;
    }
    ...
    private List<PeakReadingTimeItem> buildBookPeakReadingTimes(List<ReadingSession> sessions) {
        ...
        return minutesByHour.entrySet().stream()
                .map(e -> {
                    ...
                    item.setHour(e.getKey()); item.setMinutesRead(e.getValue());
