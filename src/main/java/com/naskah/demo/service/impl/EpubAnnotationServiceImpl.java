...
public class EpubAnnotationServiceImpl implements EpubAnnotationService {
    ...
    /**
     * Resolve slug ke content ID.
     * Return array [bookId, zineId] — salah satunya 0 jika tidak applicable.
     */
    ...
    private boolean isBook(long[] ids) { return ids[0] > 0; }
            ...
            if (existingSessions == 0) {
                ...
                if (progress != null) {
                    ...
                    response.setLastChapterIndex(progress.getCurrentPage() != null
                            ? progress.getCurrentPage() - 1 : null);
    ...
    public DataResponse<Void> recordEpubSession(String slug, EpubSessionRequest request) {
        ...
        String chapterLabel     = (request.getChapterLabel() != null && !request.getChapterLabel().isBlank())
        ...
        boolean progressIsAccurate = request.getProgressIsAccurate() != null ? request.getProgressIsAccurate() : true;
        String  lastCfi            = (request.getLastCfi() != null && !request.getLastCfi().isBlank()) ? request.getLastCfi() : null;

        // Resolve existing progress
        ReadingProgress existingProgress = isBook(ids)
                ? readingProgressMapper.findByUserAndBook(user.getId(), ids[0])
                : readingProgressMapper.findByUserAndZine(user.getId(), ids[1]);

        double previousPct      = existingProgress != null && existingProgress.getPercentageCompleted() != null
                ? existingProgress.getPercentageCompleted().doubleValue() : 0.0;
        ...
        if (existingProgress == null) {
            String newStatus = currentPct >= 95.0 ? "completed" : currentPct > 0 ? "reading" : "started";
            ...
            log.info("EPUB progress created: user={} slug={} type={} pct={}% status={}",
                    user.getId(), slug, isBook(ids) ? "book" : "zine", currentPct, newStatus);

        } else {
            double  existingPct = existingProgress.getPercentageCompleted() != null
                    ? existingProgress.getPercentageCompleted().doubleValue() : 0.0;
            int     prevMins    = existingProgress.getReadingTimeMinutes() != null
                    ? existingProgress.getReadingTimeMinutes() : 0;

            boolean shouldUpdateProgress = progressIsAccurate
                    ? currentPct > existingPct
                    : currentPct > existingPct && existingPct == 0.0;
            ...
            if (shouldUpdateProgress) {
                ...
                log.info("EPUB progress updated: user={} slug={} {}%->{}% status={}",
                        user.getId(), slug, existingPct, currentPct, newStatus);
            } else {
                existingProgress.setReadingTimeMinutes(prevMins + addedMins);
                existingProgress.setLastReadAt(endedAt);
        ...
        log.info("EPUB session recorded: user={} slug={} type={} duration={}s progress={}%",
                user.getId(), slug, isBook(ids) ? "book" : "zine", durationSeconds, currentPct);
