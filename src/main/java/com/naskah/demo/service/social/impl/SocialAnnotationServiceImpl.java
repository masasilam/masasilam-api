...
import com.naskah.demo.mapper.ZineMapper;
import com.naskah.demo.mapper.social.SocialAnnotationMapper;
...
public class SocialAnnotationServiceImpl implements SocialAnnotationService {
    ...
    public DataResponse<SocialAnnotationResponse> publishAnnotation(PublishAnnotationRequest request) {
        ...
        List<SocialAnnotationResponse> results =
                annotationMapper.findByUser(me.getId(), me.getId(), 0, 1);
    ...
    @Override
    @Transactional
    public DataResponse<SocialAnnotationResponse> updateAnnotation(
            Long annotationId, PublishAnnotationRequest request) {
        User me = requireAuth();
        ...
        List<SocialAnnotationResponse> results =
                annotationMapper.findByUser(me.getId(), me.getId(), 0, 50);
    ...
    public DataResponse<SocialAnnotationResponse> getAnnotationDetail(Long annotationId) {
        ...
        List<SocialAnnotationResponse> results =
                annotationMapper.findPublicAnnotations(currentUserId, 0, 1000);
        SocialAnnotationResponse response = results.stream()
                .filter(r -> r.getId().equals(annotationId)).findFirst()
                .orElseThrow(DataNotFoundException::new);
        List<AnnotationCommentResponse> comments =
                annotationMapper.findCommentsByAnnotation(annotationId, currentUserId);
    ...
    public DatatableResponse<SocialAnnotationResponse> getPublicAnnotations(int page, int limit) {
        ...
        List<SocialAnnotationResponse> items =
                annotationMapper.findPublicAnnotations(currentUserId, offset, limit);
    ...
    public DatatableResponse<SocialAnnotationResponse> getFollowingAnnotations(int page, int limit) {
        ...
        int offset = (page - 1) * limit;
        List<SocialAnnotationResponse> items =
                annotationMapper.findFollowingAnnotations(me.getId(), offset, limit);
    ...
    @Override
    public DatatableResponse<SocialAnnotationResponse> getUserAnnotations(
            Long userId, int page, int limit) {
        Long currentUserId = currentUserIdOrNull();
        int offset = (page - 1) * limit;
        List<SocialAnnotationResponse> items =
                annotationMapper.findByUser(userId, currentUserId, offset, limit);
    ...
    @Override
    public DatatableResponse<SocialAnnotationResponse> getEntityAnnotations(
            String entityType, Long entityId, int page, int limit) {
        Long currentUserId = currentUserIdOrNull();
        int offset = (page - 1) * limit;
        List<SocialAnnotationResponse> items =
                annotationMapper.findByEntity(entityType, entityId, currentUserId, offset, limit);
    ...
    public DataResponse<Void> reshareAnnotation(Long annotationId) {
        ...
        // create reshare as new social annotation
    ...
    @Override
    @Transactional
    public DataResponse<AnnotationCommentResponse> commentOnAnnotation(
            Long annotationId, AnnotationCommentRequest request) {
        User me = requireAuth();
    ...
    @Override
    @Transactional
    public DataResponse<AnnotationCommentResponse> updateAnnotationComment(
            Long commentId, AnnotationCommentRequest request) {
        User me = requireAuth();
    ...
    @Override
    public DatatableResponse<AnnotationCommentResponse> getAnnotationComments(
            Long annotationId, int page, int limit) {
        Long currentUserId = currentUserIdOrNull();
        List<AnnotationCommentResponse> comments =
                annotationMapper.findCommentsByAnnotation(annotationId, currentUserId);
