...
import com.naskah.demo.mapper.social.ReadingGroupMapper;
...
public class ReadingGroupServiceImpl implements ReadingGroupService {
    ...
    private void requireGroupRole(Long groupId, Long userId, String... roles) {
        ...
        boolean ok = false;
        for (String r : roles) { if (r.equals(myRole)) { ok = true; break; } }
...
// ── GROUP CRUD ────────────────────────────────────────────────────────────
    ...
    public DataResponse<ReadingGroupResponse> createGroup(CreateGroupRequest request) {
        ...
        if (request.getTags() != null && !request.getTags().isEmpty()) {
            ...
            group.setTags(null); // Jangan set string kosong, gunakan null
        }
    ...
    @Override
    public DatatableResponse<ReadingGroupResponse> getPublicGroups(
            String search, String focusType, int page, int limit) {
        Long currentUserId = currentUserIdOrNull();
        int offset = (page - 1) * limit;
        List<ReadingGroupResponse> groups =
                groupMapper.findPublicGroups(search, focusType, currentUserId, offset, limit);
    ...
    public DataResponse<Void> leaveGroup(Long groupId) {
        ...
        if ("owner".equals(member.getRole())) throw new BadRequestException("Owner cannot leave. Transfer ownership or delete the group.");
    ...
    @Override
    @Transactional
    public DataResponse<GroupReadingScheduleResponse> setCurrentRead(
            Long groupId, SetCurrentReadRequest request) {
        User me = requireAuth();
        requireGroupRole(groupId, me.getId(), "owner", "moderator");
        ReadingGroup group = groupMapper.findById(groupId);
        if (group == null) throw new DataNotFoundException();
    ...
    @Override
    @Transactional
    public DataResponse<GroupReadingScheduleResponse> createSchedule(
            Long groupId, CreateScheduleRequest request) {
        User me = requireAuth();
        requireGroupRole(groupId, me.getId(), "owner", "moderator");
    ...
    @Override
    @Transactional
    public DataResponse<GroupDiscussionResponse> createDiscussion(
            Long groupId, CreateDiscussionRequest request) {
        User me = requireAuth();
        ...
        // notify group members (simplified: notify owner only)
        ReadingGroup group = groupMapper.findById(groupId);
        ...
        List<GroupDiscussionResponse> responses =
                groupMapper.findDiscussionsByGroup(groupId, null, me.getId(), 0, 1);
    ...
    @Override
    @Transactional
    public DataResponse<GroupDiscussionResponse> updateDiscussion(
            Long groupId, Long discussionId, CreateDiscussionRequest request) {
        User me = requireAuth();
        ...
        List<GroupDiscussionResponse> responses =
                groupMapper.findDiscussionsByGroup(groupId, null, me.getId(), 0, 50);
    ...
    @Override
    public DatatableResponse<GroupDiscussionResponse> getDiscussions(
            Long groupId, Long scheduleId, int page, int limit) {
        Long currentUserId = currentUserIdOrNull();
        int offset = (page - 1) * limit;
        List<GroupDiscussionResponse> discussions =
                groupMapper.findDiscussionsByGroup(groupId, scheduleId, currentUserId, offset, limit);

        // attach replies
        discussions.forEach(d -> {
            List<GroupDiscussionResponse> replies =
                    groupMapper.findRepliesByParent(d.getId(), currentUserId);
    ...
    public DataResponse<GroupPollResponse> votePoll(Long groupId, Long pollId, VotePollRequest request) {
        ...
        if (existing != null) {
            // update vote — decrement old, increment new
            try {
                List<Map<String, Object>> options =
                        objectMapper.readValue(poll.getOptions(), List.class);
                options.forEach(opt -> {
                    int id = (int) opt.get("id");
                    if (id == existing.getOptionId()) opt.put("vote_count", Math.max(0, (int) opt.get("vote_count") - 1));
    ...
    @Override
    @Transactional
    public DataResponse<Void> reviewJoinRequest(Long groupId, Long requestId,
                                                ReviewJoinRequestRequest request) {
        User me = requireAuth();
        requireGroupRole(groupId, me.getId(), "owner", "moderator");
        ...
        if ("approve".equals(request.getAction())) {
            // find the requester userId from DB
            List<GroupJoinRequestResponse> pending = groupMapper.findPendingRequests(groupId);
            // add as member (we need userId — fetch separately)
            // simplified: the request entity has userId
            GroupJoinRequest entity = new GroupJoinRequest();
            entity.setGroupId(groupId);
            ReadingGroupMember member = new ReadingGroupMember();
            member.setGroupId(groupId);
            member.setRole("member");
            // Note: in production fetch userId from joinReq entity using findById
            groupMapper.insertMember(member);
