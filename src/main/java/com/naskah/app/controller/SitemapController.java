...
import com.naskah.app.service.book.BookService;
...
public class SitemapController {
        ...
        try {
            ...
            for (Genre genre : genres) {
                if (genre.getBookCount() != null && genre.getBookCount() >= 1) {
                    String lastmod = genre.getCreatedAt() != null ? genre.getCreatedAt().toString().substring(0, 10) : today;
    ...
    public String sitemapAuthors() {
        ...
        try {
            ...
            for (Author author : authors) {
                String lastmod = author.getUpdatedAt() != null ? author.getUpdatedAt().format(DATE_FORMATTER) : (author.getCreatedAt() != null ? author.getCreatedAt().format(DATE_FORMATTER) : LocalDateTime.now().format(DATE_FORMATTER));
    ...
    public String sitemapBooks() {
        ...
        try {
            ...
            for (Book book : books) {
                String lastmod = book.getUpdatedAt() != null ? book.getUpdatedAt().format(DATE_FORMATTER) : (book.getCreatedAt() != null ? book.getCreatedAt().format(DATE_FORMATTER) : LocalDateTime.now().format(DATE_FORMATTER));
                xml.append("  <url>\n");
    ...
    public String sitemapChapters() {
        ...
        try {
            List<Book> books = bookService.getAllBooksForSitemap();
            for (Book book : books) {
                String bookLastmod = book.getUpdatedAt() != null ? book.getUpdatedAt().format(DATE_FORMATTER) : LocalDateTime.now().format(DATE_FORMATTER);
                List<String> chapterPaths = bookService.getChapterPaths(book.getSlug());
                for (String path : chapterPaths) {
                    if (path != null && !path.isEmpty()) {
                        addUrl(xml, "/buku/" + book.getSlug() + "/" + path, "yearly", "0.6", bookLastmod);
                    }
                }
            }
        } catch (Exception e) {
