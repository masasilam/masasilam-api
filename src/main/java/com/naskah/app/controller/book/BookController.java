...
public class BookController {
    ...
    @GetMapping("/{slug}")
    public ResponseEntity<DataResponse<BookResponse>> getBookDetail(@PathVariable String slug, HttpServletRequest request) throws NoSuchAlgorithmException {
    ...
    @GetMapping("/series/{seriesSlug}")
    public ResponseEntity<DatatableResponse<BookResponse>> getBooksBySeries(@PathVariable String seriesSlug,
                                                                            @RequestParam(defaultValue = "1") @Min(1) int page,
                                                                            @RequestParam(defaultValue = "50") @Min(1) int limit) {
        DatatableResponse<BookResponse> response = bookService.getBooksBySeries(seriesSlug, page, limit);
        return ResponseEntity.ok(response);
    }
}