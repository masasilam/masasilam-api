...
/**
 * Provider untuk Internet Archive (archive.org).
 *
 * Fitur:
 *  - Embed via iframe archive.org/embed/{identifier}
 *  - Ambil direct URL (MP4) dari Metadata API archive.org/metadata/{identifier}
 *  - Deteksi berbagai kualitas (360p, 480p, 720p, 1080p) jika tersedia
 *  - Thumbnail dari archive.org/services/img/{identifier}
 *
...
public class ArchiveOrgVideoProvider implements VideoProvider {
    ...
    private static final Pattern ARCHIVE_PATTERN = Pattern.compile(
            "archive\\.org/(?:details|embed|download)/([^/?&\\s]+)"
    );

    // Format yang diprioritaskan untuk video
    private static final List<String> PREFERRED_FORMATS = List.of(
            "h.264", "mpeg4", "mp4", "512kb mpeg4", "ogg video", "webm"
    );
    ...
    public VideoMetadata resolve(String url) {
        ...
        // Panggil Metadata API untuk mendapat direct URL dan kualitas
    ...
    private void enrichFromMetadataApi(VideoMetadata.VideoMetadataBuilder builder, String identifier) {
        try {
            ...
            // Parse file list untuk direct URL dan kualitas
    ...
    private int rankFormat(String fmt, String name) {
        // Makin kecil angka = makin diutamakan
    ...
    private Map<String, String> sortQualities(Map<String, String> qualities) {
        // Sort by numeric height descending (1080p > 720p > 480p > 360p)
    ...
    private int parseHeight(String label) {
        try { return Integer.parseInt(label.replace("p", "")); }
