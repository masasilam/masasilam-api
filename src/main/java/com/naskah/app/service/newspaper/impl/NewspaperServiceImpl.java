package com.naskah.app.service.newspaper.impl;

import com.naskah.app.exception.custom.*;
import com.naskah.app.mapper.*;
import com.naskah.app.model.dto.newspaper.*;
import com.naskah.app.model.dto.response.*;
import com.naskah.app.model.entity.User;
import com.naskah.app.model.entity.newspaper.*;
import com.naskah.app.service.newspaper.NewspaperService;
...
public class NewspaperServiceImpl implements NewspaperService {
    ...
    public DataResponse<NewspaperArticleDetailResponse> updateArticle(Long id, UpdateArticleRequest request) {
        try {
            ...
            // htmlContent — fallback ke existing jika tidak dikirim
            ...
            // ✅ FIX SOURCE: resolveSourceId — jika sourceId dikirim, langsung pakai
            Long sourceId = resolveSourceId(request.getSourceId(), request.getSourceName());
            if (sourceId != null) {
                existing.setSourceId(sourceId);
            }
            // Jika keduanya null → pertahankan sourceId existing
            ...
            // ✅ FIX SLUG: update slug jika dikirim dan tidak sama dengan yang ada
            ...
            // Fetch ulang dengan slug terbaru (mungkin sudah berubah)
    ...
    private void enrichArticleResponse(NewspaperArticleResponse article, Long userId) {
        ...
        if (userId != null) {
            try { article.setIsSaved(newspaperMapper.isArticleSavedByUser(article.getId(), userId)); }
            catch (Exception e) { log.warn("isArticleSavedByUser failed for {}: {}", article.getId(), e.getMessage()); }
            try {
            ...
            } catch (Exception e) { log.warn("articleRating failed for {}: {}", article.getId(), e.getMessage()); }
        }
    }
    ...
    private void enrichArticleDetailResponse(NewspaperArticleDetailResponse detail, Long userId) {
        ...
        // ✅ FIX SOURCE: bangun object NewspaperSourceResponse dari flat fields yang sudah ada di detail
        // Query SQL mengembalikan sourceId, sourceName, sourceLocation, sourceDescription sebagai flat fields
        // tapi DTO punya field "source" bertipe NewspaperSourceResponse — perlu dibangun manual
        if (detail.getSourceId() != null) {
            ...
            sourceObj.setName(detail.getSourceName());
        ...
        if (userId != null) {
            try { detail.setIsSaved(newspaperMapper.isArticleSavedByUser(detail.getId(), userId)); }
            catch (Exception e) { log.warn("isArticleSavedByUser failed for {}: {}", detail.getId(), e.getMessage()); }
            try {
            ...
            } catch (Exception e) { log.warn("articleRating failed for {}: {}", detail.getId(), e.getMessage()); }
            try { detail.setHasReviewed(newspaperMapper.hasUserReviewedArticle(userId, detail.getId())); }
            catch (Exception e) { log.warn("hasUserReviewedArticle failed for {}: {}", detail.getId(), e.getMessage()); }
        }
    }

    private List<NewspaperArticleResponse> getRelatedArticles(Long articleId, String category, int limit) {
        try { return newspaperMapper.getRelatedArticles(articleId, category, limit); }
        catch (Exception e) { log.error("Error getting related articles", e); return Collections.emptyList(); }
    }

    private List<NewspaperArticleResponse> getSameDateArticles(Long articleId, LocalDate date, int limit) {
        try { return newspaperMapper.getSameDateArticles(articleId, date, limit); }
        catch (Exception e) { log.error("Error getting same date articles", e); return Collections.emptyList(); }
    }
