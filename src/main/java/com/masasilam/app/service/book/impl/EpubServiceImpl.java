...
import com.masasilam.app.model.dto.ChapterHierarchy;
...
public class EpubServiceImpl implements EpubService {
    ...
    @Override
    @ClearChapterCache
    public EpubProcessResult processEpubFile(MultipartFile epubFile, Book book, ChapterRepository chapterRepository) throws IOException {
        ...
        try (InputStream is = epubFile.getInputStream()) {
            ...
            List<BookChapter> chapters = extractAndSaveChaptersWithHierarchy(epubBook, book.getId(), new HashMap<>(), chapterRepository);
    ...
    @Override
    @ClearChapterCache
    public EpubProcessResult processEpubFileForUpdate(MultipartFile epubFile, Book book, ChapterRepository chapterRepository) throws IOException {
        ...
        try (InputStream is = epubFile.getInputStream()) {
            ...
            for (BookChapter existingChapter : existingChapters) {
                if (!processedChapterNumbers.contains(existingChapter.getChapterNumber())) {
                    ...
                    log.info("Deleted obsolete chapter: {} (ID: {})", existingChapter.getTitle(), existingChapter.getId());
                }
            }
    ...
    private void processImages(nl.siegmann.epublib.domain.Book epubBook, Document doc, Long entityId, Map<String, String> imageCache) {
        Elements imgElements = doc.select("img");
        for (Element img : imgElements) {
            String imgSrc = img.attr("src");
            if (!imgSrc.isEmpty()) {
                try {
                    String cloudinaryUrl = extractAndUploadChapterImageOptimized(epubBook, imgSrc, entityId, imageCache);
    ...
    private String[] extractContent(Document doc, String anchorId, int i, List<ChapterHierarchy> orderedToc, String fileName) {
        if (anchorId != null) {
    ...
    private String[] extractAnchoredContent(Document doc, String anchorId, int i, List<ChapterHierarchy> orderedToc, String fileName) {
    ...
    private String[] extractRootContent(Document doc, int i, List<ChapterHierarchy> orderedToc, String fileName) {
