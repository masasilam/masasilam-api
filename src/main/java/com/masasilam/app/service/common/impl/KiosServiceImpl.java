...
import com.masasilam.app.mapper.kios.*;
...
import com.masasilam.app.model.dto.FileStorageResult;
...
public class KiosServiceImpl implements KiosService {
    ...
    private static final String PHYSICAL_BOOK = "PHYSICAL_BOOK";
    ...
    private static final String SOLD_COUNT = "SOLD_COUNT";
    ...
    public DataResponse<KiosOrderResponse> createOrder(CreateOrderRequest request) {
        try {
            ...
            for (CartItem item : cartItems) {
                ...
                createStockHistory(product.getId(), -item.getQuantity(), ORDER, "Order created", orderNumber);
            }
    ...
    public DatatableResponse<KiosOrderResponse> getUserOrders(int page, int limit, String status) {
        try {
            ...
            List<KiosOrderResponse> orders = orderMapper.findOrdersByUserId(user.getId(), status, offset, limit);
    ...
    public DataResponse<KiosOrderResponse> cancelOrder(String orderNumber) {
        try {
            ...
            for (OrderItem item : items) {
                ...
                createStockHistory(product.getId(), item.getQuantity(), CANCELLED, "Order cancelled", orderNumber);
            }
    ...
    public DatatableResponse<KiosOrderResponse> getAllOrders(int page, int limit, String status, String search) {
        try {
            ...
            int offset = (page - 1) * limit;
            List<KiosOrderResponse> orders = orderMapper.findAllOrdersWithFilters(status, search, offset, limit);
    ...
    private void createStockHistory(Long productId, Integer quantityChange, String type, String reason, String orderNumber) {
    ...
    private CartResponse getCartResponse(Long userId) {
        ...
        List<CartResponse.CartItemResponse> itemResponses = items.stream()
                .map(item -> {
                    ...
                    itemResponse.setPrice(product.getDiscountPrice() != null ? product.getDiscountPrice() : product.getPrice());
                    itemResponse.setQuantity(item.getQuantity());
                    itemResponse.setSubtotal(itemResponse.getPrice().multiply(BigDecimal.valueOf(item.getQuantity())));
    ...
    private String generateOrderNumber() {
        return "KIO" + System.currentTimeMillis() + String.format("%04d", new Random().nextInt(10000));
    }

    private BigDecimal calculateShippingCost(BigDecimal weight, String province) {
        BigDecimal baseRate = new BigDecimal("15000");
        BigDecimal perKg = new BigDecimal("5000");

        BigDecimal weightCost = weight.divide(new BigDecimal("1000"), 2, RoundingMode.HALF_UP).multiply(perKg);
